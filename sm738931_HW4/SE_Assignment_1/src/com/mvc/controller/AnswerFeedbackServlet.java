...
public class AnswerFeedbackServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ans.equalsIgnoreCase(Qb.getAnswer()))
		{
			...
			RequestDispatcher requestDispatcher = getServletContext().getRequestDispatcher("/SolveQuestionFeedback.jsp");
		...
		else
		{
			...
			request.getSession().setAttribute("uans", ans);
			RequestDispatcher requestDispatcher = getServletContext().getRequestDispatcher("/solveQuestionHint.jsp");
			requestDispatcher.forward(request, response);
		}
